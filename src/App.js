...
class App extends Component {
  ...
  matchItem(selectedCategory, searchQuery) {
    ...
    if (selectedCategory === "all" && searchQuery !== "") {
      updatedFilteredList = this.state.list.filter(
        listItem => listItem.title.toUpperCase().indexOf(searchQuery.toUpperCase()) > -1
      );
    }
    if (selectedCategory !== "all" && searchQuery === "") {
    ...
    if (selectedCategory !== "all" && searchQuery !== "") {
      updatedFilteredList = this.state.list.filter(
        listItem =>
          listItem.category === selectedCategory && listItem.title.toUpperCase().indexOf(searchQuery.toUpperCase()) > -1
      );
    }
    ...
    let newListItem = {
      id:
        this.state.selectedItemId !== null
          ...
          : Math.max.apply(Math, this.state.list.map(item => item.id)) + 1,
    ...
    if (isMobile) {
      return (
        <main>
          ...
          <List
            list={
              this.state.selectedCategory !== "all" || this.state.searchQuery
          ...
          <Controls>
            ...
            />
            <Button className="Add-Item" name="Add New" clicked={this.handleOpenModal} />
          </Controls>

          <Backdrop showBackdrop={this.state.modalOpened || this.state.showWarning}>
            <Modal
